 ...
 * I don't particularly have the dopamine relaxation to favor the existence of the april tags location relative to the 
 * location of the very much so desired positions of dedicated robot void.
 */
public class AutoAlignToAprilTagCommand extends Command {
    ...
    void execute() {
        ...
        double yError = limelightSubsystem.getClosestTY();      // Vertical error (in degrees) (not needed for E)
        ...
        double yRotationCorrection = -kP_rotation * yError; //IDK WHAT THIS DOES
        // Compute the rotation correction ()
    ...
    public boolean isFinished() {
        ...
        // TODO: cancel the command when limelight.tx is at a setpoint/desired distance, or at the middle of the screen
        if (!limelightSubsystem.getTV()) {
            return false;
        }
